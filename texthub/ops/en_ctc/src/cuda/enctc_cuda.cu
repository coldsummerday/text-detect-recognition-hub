...
#endif
enctc_loss_log_alpha_gpu_kernel(scalar_t* __restrict__ log_alpha_data,
                                ...
                                scalar_t* __restrict__ log_ent_data,
                                scalar_t* __restrict__ log_alpha_ent_data,
  ...
  float eps = 1e-6;
  float minlogvalue = std::log(0.000001);
  if (b >= batch_size)
    return;
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += blockDim.x) {
    ...
    switch (s) {
      case 0:
        ...
        //p(x)*log(p(x)),log(p(x))=la;
        //TODO:这里数值太小会导致nan,所以初始化的时候不能是neginf， neginf log(neginf)=0.0 ，合理设置xlogx 初始值问题
        la_ent  = la + std::log(-la+eps);
        ...
        la = target_length == 0 ? neginf
             ...
             [lp_batch_offset +
              ...
              get_target_prime(
                  ...
                  1,
                  BLANK)];
        la_ent = target_length==0?minlogvalue:la + std::log(-la+eps);
    ...
    if (s < 2*max_target_length+1)
      ...
      log_alpha_ent_data[la_batch_offset + /* la_input_stride * 0 */ + la_target_stride * s] = la_ent;
  }
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += blockDim.x) {
    ...
    for (int64_t t=1; t < max_input_length; t++) {
      ...
      if ((t < input_length) && (s < 2 * target_length + 1)) {
        ...
        scalar_t la1_ent = log_alpha_ent_data[la_batch_offset + la_input_stride * (t-1) + la_target_stride * s];

        scalar_t laentmax = la1_ent;
        scalar_t lamax = la1;
        scalar_t la2, la3,la2_ent,la3_ent;
        if (s > 0) {
          la2 = log_alpha_data[la_batch_offset + la_input_stride * (t-1) + la_target_stride * (s-1)];
          la2_ent = log_alpha_ent_data[la_batch_offset + la_input_stride * (t-1) + la_target_stride * (s-1)];
          if (la2 > lamax)
            lamax = la2;
          if (la2_ent > laentmax)
            laentmax = la2_ent;
        ...
        if (have_three) {
          ...
          la3_ent = log_alpha_ent_data[la_batch_offset + la_input_stride * (t-1) + la_target_stride * (s-2)];
          if (la3 > lamax)
            lamax = la3;
          if (la3_ent > laentmax)
            laentmax = la3_ent;
        ...
        if (lamax == neginf) // when all are neginf. (then the whole thing is neginf, but we can pretend)
          lamax = 0;
        if (laentmax ==neginf)
            laentmax = minlogvalue;
        

        //原来ctc 前向概率
        scalar_t log_alpha_value = std::log(std::exp(la1-lamax)+std::exp(la2-lamax)+std::exp(la3-lamax))+lamax + log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_char];
        log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s] = log_alpha_value;
        
        //添加此处作为前向概率最大熵部分
        scalar_t log_alpha_ent_value = std::log(std::exp(la1_ent-laentmax)+std::exp(la2_ent-laentmax)+std::exp(la3_ent-laentmax))+laentmax+ log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_char];                                                                               
      
        //y(t,s)(*p) + alpha(t,s)*log (*p)
        //alpha(t,s)log(p)为相乘的形式，所以是相加
        scalar_t ent_value = std::log(-log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_char])+log_alpha_value;
        log_alpha_ent_value = safe_log_add(log_alpha_ent_value,ent_value);
        log_alpha_ent_data[la_batch_offset + la_input_stride * t + la_target_stride * s] = log_alpha_ent_value;
        // printf("log_alpha:%f\n",log_alpha_ent_data);
      } else {
        // otherwise we just set to neginf
        if (s < 2*max_target_length+1)
          log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s] = neginf;
          log_alpha_ent_data[la_batch_offset + la_input_stride * t + la_target_stride * s] = minlogvalue;
      }
    }
  }
  __syncthreads(); // on cuda 9 we might use partial synchronization of only the threads within the same batch
  ...
  if (threadIdx.x == 0) {
    ...
    scalar_t l1_ent = log_alpha_ent_data[la_batch_offset + la_input_stride * (input_length-1) + la_target_stride * (target_length*2)];
    scalar_t l2_ent = target_length > 0
                  ? log_alpha_ent_data
                  [la_batch_offset + la_input_stride * (input_length - 1) +
                   la_target_stride * (target_length * 2 - 1)]
                  : neginf;
    scalar_t m_ent = ((l1_ent > l2_ent) ? l1_ent : l2_ent);
    m_ent = ((m_ent == minlogvalue) ? 0 : m_ent);

    //log(Q(l))
    scalar_t log_ent = std::log(std::exp(l1_ent-m_ent)+std::exp(l2_ent-m_ent))+m_ent;
    log_ent_data[b] = log_ent;
  }
}
...
std::tuple<at::Tensor, at::Tensor,at::Tensor,at::Tensor> enctc_loss_gpu_template(const at::Tensor& log_probs, const at::Tensor& targets, at::IntArrayRef input_lengths, at::IntArrayRef target_lengths, int64_t BLANK) {
  // log_probs: input_len x batch_size x num_labels
  // targets [int64]: batch_size x target_length OR sum(target_lengths)
  CheckedFrom c = "enctc_loss_gpu";
  ...
  auto target_lengths_t = at::tensor(target_lengths, targets.options().dtype(at::kLong));
  auto input_lengths_t = at::tensor(input_lengths, targets.options().dtype(at::kLong));
  tg_batch_offsets = tg_batch_offsets.cuda();

  at::Tensor log_alpha = at::empty({batch_size, log_probs.size(0), 2*max_target_length+1}, log_probs.options());
  at::Tensor neg_log_likelihood = at::empty({batch_size}, log_probs.options());

  at::Tensor log_alpha_ent = at::empty({batch_size, log_probs.size(0), 2*max_target_length+1}, log_probs.options());
  at::Tensor log_ent = at::empty({batch_size}, log_probs.options());
  ...
  enctc_loss_log_alpha_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>(
      ...
      log_ent.data_ptr<scalar_t>(),
      log_alpha_ent.data_ptr<scalar_t>(),
...
//   TORCH_CUDA_KERNEL_LAUNCH_CHECK();
  return std::make_tuple(neg_log_likelihood, log_alpha,log_ent,log_alpha_ent);
}
...
__global__ void
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
enctc_loss_backward_log_beta_gpu_kernel(scalar_t* __restrict__ log_beta_data,
                                      const scalar_t*log_probs_data, const int64_t* __restrict__ input_lengths, int64_t max_input_length,
                                      const target_t* __restrict__ targets_data, const int64_t* __restrict__ target_lengths, int64_t max_target_length,
                                      int64_t lp_input_stride, int64_t lp_batch_stride, int64_t lp_char_stride,
...
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
#endif
enctc_loss_backward_collect_nonblank_gpu_kernel(scalar_t* __restrict__ gradient_data,
                                                     ...
                                                     const scalar_t* __restrict__ log_ent_data,
                                                     const scalar_t* __restrict__ log_alpha_ent_data,
                                                     int64_t gr_input_stride, int64_t gr_batch_stride, int64_t gr_char_stride,
                                                     int64_t lp_input_stride, int64_t lp_batch_stride, int64_t lp_char_stride,
                                                     int64_t la_batch_stride, int64_t la_input_stride, int64_t la_target_stride,
                                                     int64_t lb_batch_stride, int64_t lb_input_stride, int64_t lb_target_stride,
                                                     const int64_t* __restrict__ tg_batch_offsets, int64_t tg_target_stride,
                                              int64_t batch_size, int64_t num_labels, int64_t BLANK, bool zero_infinity,float ent_rate) {
  int64_t b = threadIdx.y + blockIdx.y * blockDim.y;
  ...
  scalar_t nll_log_ent = log_ent_data[b];
  if (zero_infinity && nll == INFINITY&&nll_log_ent==INFINITY)
    return;
  ...
  for (int64_t t = 0; t < input_length; t++) {
    ...
    scalar_t log_alpha_ent_sub_qnll = log_alpha_ent_data[la_batch_offset + la_input_stride * t + la_target_stride * (s*2+1)] + nll_log_ent;
    
    scalar_t log_alpha_sub_nll = log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * (s*2+1)] +nll;
    
    scalar_t log_ql_sub_nll_log_p =  nll_log_ent - lp +nll;
    

    //TODO：此处，h_grad 不是nan就是inf 了
    scalar_t h_grad = ent_rate * std::exp(log_ql_sub_nll_log_p +safe_log_add(log_alpha_ent_sub_qnll,-log_alpha_sub_nll))*gr;
    
    
    scalar_t& grad_res = gradient_data[gr_batch_offset + t * gr_input_stride + gr_char_stride * target];
    if(grad_res==-INFINITY){
      grad_res = 0.;
    }
    ...
    gpuAtomicAdd(&gradient_data[gr_batch_offset + t * gr_input_stride + gr_char_stride * target],
              (std::exp(lp)-std::exp(log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * (s*2+1)]
                        + log_beta_data[lb_batch_offset + lb_input_stride * t + lb_target_stride * (s*2+1)]
                        + nll - lp)) * gr);
    //entroy loss grad
    gpuAtomicAdd(&gradient_data[gr_batch_offset + t * gr_input_stride + gr_char_stride * target],h_grad);
  }
}
...
C10_LAUNCH_BOUNDS_2((std::is_same<scalar_t, float>::value ? 1024 : 896), 1)
#endif
enctc_loss_backward_collect_gpu_kernel(scalar_t* __restrict__ gradient_data,
                                                     const scalar_t* __restrict__ grad_out_data, int64_t grad_out_batch_stride,
                                                     const scalar_t* __restrict__ log_alpha_data, const scalar_t* __restrict__ log_beta_data,
                                                     const scalar_t*log_probs_data, const int64_t* __restrict__ input_lengths, int64_t max_input_length,
                                                     const target_t* __restrict__ targets_data, const int64_t* __restrict__ target_lengths, int64_t max_target_length,
  ...
  scalar_t nll = neg_log_likelihood_data[b];
  scalar_t gr =  grad_out_data[b * grad_out_batch_stride];
  scalar_t log_ql = log_ent_data[b];
  float minlogvalue = std::log(0.000001);
  // const int flag_length = num_labels+1;
  bool* char_flag=new bool[num_labels+1]; 
  // collected[b, t, target'[s]]
  for (int s = 0; s < 2*max_target_length+1; s++) {
    ...
    if (s < 2 * target_length + 1) { // if target_length == 0, s == 0
      ...
      scalar_t lp_s = log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_target_prime];
      if (isinf(nll)) {
        lcab = 0.0;
        continue;
      }
      scalar_t ctc_grad = (std::exp(lp_s)-std::exp(log_alpha_beta + nll - lp_s)) ;

      scalar_t log_alpha_ent_sub_qnll = log_alpha_ent_data[la_batch_offset + la_input_stride * t + la_target_stride * s] +log_ql;
    
      
      scalar_t log_alpha_sub_nll = log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s]==-INFINITY?minlogvalue+nll:log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s]+nll;

      
      // scalar_t log_ql_sub_nll_log_p =log_ql - lp_s +nll;
      scalar_t log_ql_sub_nll_log_p =(-lp_s)*log_ql - lp_s +nll;
      // scalar_t log_ql_sub_nll_log_p =log_ql>-nll?std::log(nll)-lp_s   :log_ql - lp_s +nll;
      // printf("log_ql:%f,lq_s:%f,nll:%f,alpha:%f,log_ql_sub_nll:%f\n",log_ql,lp_s,nll,log_alpha_beta,log_ql_sub_nll_log_p);
      scalar_t h_grad = std::exp(-log_ql_sub_nll_log_p +safe_log_add(log_alpha_ent_sub_qnll,-log_alpha_sub_nll));
      //h_grad 过小导致float 越界了
      // printf("ctc grad :%f,h_grad:%f,l1:%f,nll:%f,l2:%f,l3:%f,l4:%f\n",ctc_grad,h_grad,log_alpha_ent_sub_qnll,nll,log_alpha_sub_nll,log_ql_sub_nll_log_p,safe_log_add(log_alpha_ent_sub_qnll,-log_alpha_sub_nll));
      if(h_grad>1||h_grad<-1){
        h_grad = ctc_grad * lp_s + std::exp(lp_s) * 1/ctc_grad;

      }
      if (lcab == 0.0) {
        lcab = (ctc_grad + ent_rate * h_grad)*gr;
      } else {
        lcab += (ctc_grad + ent_rate * h_grad)*gr;
        // scalar_t max = ((lcab > log_alpha_beta) ? lcab : log_alpha_beta);
        // lcab = std::log(std::exp(lcab-max)+std::exp(log_alpha_beta-max))+max;
      }
      char_flag[current_target_prime] = true;
    }
  }
  

  //非target 路径上的c的loss 为lp，最大熵部分梯度也为0，只有原ctcloss 部分的概率p
...
template<typename scalar_t, at::ScalarType target_scalar_type>
Tensor enctc_loss_backward_gpu_template(const Tensor& grad_out, const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths,
                                      const Tensor& neg_log_likelihood, const Tensor& log_alpha,const Tensor& log_ent, const Tensor& log_alpha_ent,int64_t BLANK, bool zero_infinity,float ent_rate)
                                       {
  constexpr scalar_t neginf = -INFINITY;
  using target_t = typename std::conditional<target_scalar_type == kInt, int, int64_t>::type;
  int64_t batch_size = log_probs.size(1);
  ...
  Tensor grad = at::full_like(log_probs, 0); // initialization for 0
  ...
  //计算后验概率
  {
    dim3 block(threads_target, threads_batch);
    dim3 grid((2*max_target_length+1 + threads_target-1)/threads_target, (batch_size+threads_batch-1)/threads_batch);
    enctc_loss_backward_log_beta_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>
  ...
  if (is_large) { // large alphabet, large batch
    // this computes the probs, minuend in (16)
      //enctc 中有两项loss，所以不能初始化的时候就把所有梯度设置为log_prob,只有target中没有的字符的梯度才是log_prob
    ...
    grad_blank =(log_probs.narrow(2, BLANK, 1).exp_() - (at::logsumexp(log_alpha.as_strided({batch_size, log_alpha.size(1), max_target_length+1},
    ...
    // //  减去最大熵loss
    // at::Tensor log_alpha_add_nll = log_alpha.as_strided({batch_size, log_alpha.size(1), max_target_length+1},
    // {log_alpha.stride(0), log_alpha.stride(1), log_alpha.stride(2)*2}).permute({1,0,2}).add_(neg_log_likelihood.view({1, batch_size, 1}));
    // at::Tensor log_alpha_ent_sub_qnll = log_alpha_ent.as_strided({batch_size, log_alpha.size(1), max_target_length+1},
    // {log_alpha.stride(0), log_alpha.stride(1), log_alpha.stride(2)*2}).permute({1,0,2}).sub_(log_ent.view({1, batch_size, 1}));
    
 
    // grad_blank += ent_rate * at::logsumexp(log_alpha_ent_sub_qnll-log_alpha_add_nll,2,true)
    // .add_(neg_log_likelihood.view({1,batch_size,1}))
    // .add_(log_ent.view({1, batch_size, 1}))
    // .sub_(log_probs.narrow(2, BLANK, 1))
    // .exp_();
    ...
    // std::cout<<grad_blank<<std::endl; //可以打印出数值
    // std::cout<<grad.narrow(2,BLANK,1)<<std::endl; //可以打印出数值
    ...
    dim3 grid(
        ...
        1);
    enctc_loss_backward_collect_nonblank_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>
      (grad.data_ptr<scalar_t>(),
       ...
       log_beta.stride(0), log_beta.stride(1), log_beta.stride(2),
       tg_batch_offsets.data_ptr<int64_t>(), tg_target_stride,
       batch_size, num_labels, BLANK, zero_infinity, ent_rate);
    // TORCH_CUDA_KERNEL_LAUNCH_CHECK();

  } else { // small problem, use naive algorithm
    // TODO:batch_size 设置小的时候走这条路径
    ...
    enctc_loss_backward_collect_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>
      (grad.data_ptr<scalar_t>(),
       grad_out.data_ptr<scalar_t>(), grad_out.stride(0),
       log_alpha.data_ptr<scalar_t>(), log_beta.data_ptr<scalar_t>(),
       log_probs.data_ptr<scalar_t>(), input_lengths_t.data_ptr<int64_t>(), log_probs.size(0),
  ...
  //非法值 置0 处理
  grad = at::where(grad == Scalar(INFINITY) , at::zeros({}, grad.options()), grad);
  grad = at::where(at::isnan(grad) , at::zeros({}, grad.options()), grad);
  return grad;
}
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> enctc_loss_gpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(log_probs.scalar_type(), "enctc_loss_gpu", [&] {
      if (targets.scalar_type() == kLong) {
        return enctc_loss_gpu_template<scalar_t, kLong>(log_probs, targets, input_lengths, target_lengths, BLANK);
      } else {
        return enctc_loss_gpu_template<scalar_t, kInt>(log_probs, targets, input_lengths, target_lengths, BLANK);
      }
    });
}
...
at::Tensor enctc_loss_backward_gpu(
  ...
  return AT_DISPATCH_FLOATING_TYPES(log_probs.scalar_type(), "enctc_loss_backward_cuda", [&] {
      if (targets.scalar_type() == kLong) {
        return enctc_loss_backward_gpu_template<scalar_t, kLong>(grad, log_probs, targets, input_lengths, target_lengths, neg_log_likelihood, log_alpha,log_ent,log_alpha_ent,BLANK, zero_infinity,ent_rate);
      } else {
        return enctc_loss_backward_gpu_template<scalar_t, kInt>(grad, log_probs, targets, input_lengths, target_lengths, neg_log_likelihood, log_alpha, log_ent,log_alpha_ent,BLANK, zero_infinity,ent_rate);
      }
    });
}
