...
ctc_loss_log_alpha_gpu_kernel(scalar_t* __restrict__ log_alpha_data,
  ...
  // int64_t char_data_batch_offset = b * num_labels;
  // int es_length = (int)((float)input_length*2 / (float)target_length);       
  // int64_t char_nums = lp_batch_stride;
  // int64_t char_frequency_array[char_nums+1];
  // memset(char_frequency_array,0, char_nums+1);                                  
  ...
  //     int64_t char_current =get_target_prime(
  //       targets_data,
  //       tg_batch_offset,
  //       tg_target_stride,
  //       s,
  ...
  //     char_es_data[char_data_batch_offset +  char_current]+= 3*es_length;
                                    ...
                                    [lp_batch_offset +
                                     lp_char_stride *
                                         get_target_prime(
                                             ...
                                             1,
                                             BLANK)];
        // char_frequency_data[char_data_batch_offset + get_target_prime(
        // targets_data,
        // tg_batch_offset,
        // tg_target_stride,
        // 1,
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += blockDim.x) {
    ...
    for (int64_t t=1; t < max_input_length; t++) {
      ...
      // {
      //   //至少有一项的时候次数+1
      //   char_frequency_data[char_data_batch_offset + current_char]++;
      // }
        ...
        // if (have_three && char_frequency_data[char_data_batch_offset+current_char]>char_es_data[char_data_batch_offset+current_char]){
          
        //     scalar_t log_po = (scalar_t)log_poisson(char_frequency_data[char_data_batch_offset+current_char],char_es_data[char_data_batch_offset+current_char]);
        //     scalar_t log_po_mean = (scalar_t)log_poisson(char_es_data[char_data_batch_offset+current_char],char_es_data[char_data_batch_offset+current_char]);
        //     log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s] += (log_po-log_po_mean);    
  ...
  // delete[] char_frequency_array;
  // delete[] char_es_length;
}
...
std::tuple<Tensor, Tensor> ctc_loss_gpu_template(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK) {
  ...
  // Tensor char_frequency_tensor = at::zeros({batch_size,num_labels}).to(kInt).to(kCUDA);
  // Tensor char_es_tensor = at::zeros({batch_size,num_labels}).to(kInt).to(kCUDA);
...
// delete[] char_frequency_array;
// cudaFree(cuda_char_frequency_array);
