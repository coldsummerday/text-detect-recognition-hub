...
__device__ static inline float log_poisson(int num, int lamda)
{
  float f=std::log(num);
  int num_int = num-1;
  while(num_int>0){
  ...
  float res = -lamda + std::log(std::pow(lamda,num));
  res -=f;
  return res;
}


__device__ static inline float poisson_regular(int num, int lamda)
{

  //math.log(0.5)
  //-0.6931471805599453
  const float min_p = 0.5;
  float log_po = log_poisson(num,lamda);
  //除以均值
  float log_po_mean =  log_poisson(lamda,lamda);
  
  float normal_regular = log_po- log_po_mean;
  float final_regu = 0.0;
  if(normal_regular<0 && normal_regular>std::log(min_p)){
    final_regu= normal_regular;
  }else{
    final_regu= log_poisson(lamda+3,lamda)-log_po_mean;
  }
  // printf("f regu:%f,%d,%d\n",final_regu,num,lamda);
  return final_regu;
}
...
// __device__ static inline float log_p_margin(float p)
// {
//   if (p)
//   int f=1;
//   int num_int = num;
...
// __device__ static inline scalar_t log_poisson_t(scalar_t num, scalar_t lamda)
// {
//   int f=1;
//   int num_int = num;
//   while(num_int>0){
...
//     num_int--;
//   }
//   scalar_t res = -lamda + std::log(std::pow(lamda,num));
//   res -=std::log(f);
//   return res;
// }
...
ctc_loss_log_alpha_gpu_kernel(scalar_t* __restrict__ log_alpha_data,
                                    ...
                                    int* __restrict__ char_frequency_data,
                                    int* __restrict__  char_es_data,
                                    int64_t char_batch_stride,int64_t fre_char_stride,
  ...
  int64_t char_data_batch_offset = b * char_batch_stride;
  int es_length = (int)((float)input_length*2 / (float)target_length);       
                         
                   
  if (b >= batch_size)
    return;
  //target 出现一次，则 增加该字符的平均长度
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += blockDim.x) {
      ...
      if (s < 2 * target_length + 1 && target_length > 0) {
        ...
        current_char = get_target_prime(
            ...
            s,
            BLANK);
        char_es_data[char_data_batch_offset +  current_char*fre_char_stride]+= 2*es_length;
        }

      //ctc 只往前看s-2,所以一次出现最多三条路径,每个路径es_length步
      // char_es_data[es_data_batch_offset +  char_current*es_char_stride]+= 3*es_length;
  }                                    
                                    ...
                                    [lp_batch_offset +
                                     lp_char_stride *
                                         get_target_prime(
                                             ...
                                             1,
                                             BLANK)];
        char_frequency_data[char_data_batch_offset + fre_char_stride* get_target_prime(
        targets_data,
        tg_batch_offset,
        tg_target_stride,
        1,
  ...
  for (int64_t block_s = 0; block_s < 2*max_target_length+1; block_s += blockDim.x) {
    ...
    for (int64_t t=1; t < max_input_length; t++) {
      ...
      if ((t < input_length) && (s < 2 * target_length + 1)) {
        ...
        //概率大于log(0.1),即-2.302585时
      if(log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_char]>-2.302585)
      {
        //至少有一项的时候次数+1
        char_frequency_data[char_data_batch_offset + fre_char_stride*current_char]++;
      }
        ...
        if (current_char!=BLANK && char_frequency_data[char_data_batch_offset+fre_char_stride*current_char]>char_es_data[char_data_batch_offset+fre_char_stride*current_char]){
          
        
            // float log_po = log_poisson(char_frequency_data[char_data_batch_offset+current_char*fre_char_stride],char_es_data[char_data_batch_offset+current_char*fre_char_stride]);
            // float log_po_mean = log_poisson(char_es_data[char_data_batch_offset+current_char*fre_char_stride],char_es_data[char_data_batch_offset+current_char*fre_char_stride]);
            // //应该写个函数来作为正则约束
            
            float po_regu = poisson_regular(char_frequency_data[char_data_batch_offset+current_char*fre_char_stride],char_es_data[char_data_batch_offset+current_char*fre_char_stride]);
            // printf("regu:%f\n",po_regu);

            log_alpha_data[la_batch_offset + la_input_stride * t + la_target_stride * s] +=po_regu;    
...
std::tuple<Tensor, Tensor> ctc_loss_gpu_template(const Tensor& log_probs, const Tensor& targets, IntArrayRef input_lengths, IntArrayRef target_lengths, int64_t BLANK) {
  ...
  Tensor char_frequency_tensor = at::empty({batch_size,num_labels+1}).fill_(0).to(kInt).to(kCUDA);
  Tensor char_es_tensor = at::empty({batch_size,num_labels+1}).fill_(0).to(kInt).to(kCUDA);


  // int* char_frequency_array = new int[batch_size*num_labels+1];
  // int* cuda_char_frequency_array;
  // cudaMalloc(&cuda_char_frequency_array, sizeof(int)*(batch_size*num_labels+1));
  // cudaMemcpy(char_frequency_array,cuda_char_frequency_array,sizeof(int)*(batch_size*num_labels+1), cudaMemcpyHostToDevice); //第一个参数为目的地址，第二个为原地址，第三个为空间大小
  

    
  // int* char_es_array = new int[batch_size*num_labels+1];
  // int* cuda_char_es_array;
  // cudaMalloc(&cuda_char_es_array, sizeof(int)*(batch_size*num_labels+1));
  // cudaMemcpy(char_es_array,cuda_char_es_array,sizeof(int)*(batch_size*num_labels+1), cudaMemcpyHostToDevice); //第一个参数为目的地址，第二个为原地址，第三个为空间大小
  ...
  constexpr int max_threads = std::is_same<scalar_t, float>::value ? 512 : 448; // we need 72 or so 32 bit registers for double
  ...
  ctc_loss_log_alpha_gpu_kernel<scalar_t, target_t><<<grid, block, 0, stream>>>(
                      ...
                      // num_labels,
                      char_frequency_tensor.stride(0),char_frequency_tensor.stride(1),
...
ctc_loss_backward_log_beta_gpu_kernel(scalar_t* __restrict__ log_beta_data,
    ...
    for (int64_t block_s = 2*max_target_length - (2*max_target_length % blockDim.x); block_s >= 0; block_s -= blockDim.x) {
      ...
      if (s < 2*max_target_length+1) {
        ...
        if (current_target_prime!=BLANK){
          char_es_data[char_data_batch_offset +  current_target_prime*fre_char_stride]+= 2*es_length;
        }
      }
    }
  ...
  for (int64_t block_s = 2*max_target_length - (2*max_target_length % blockDim.x); block_s >= 0; block_s -= blockDim.x) {
    ...
    if (s == 2*target_length) {
      ...
      char_frequency_data[char_data_batch_offset + fre_char_stride*current_target_prime ]++;
    } else {
      lb = neginf;
    }
    if (s < 2*max_target_length+1) {
  ...
  for (int64_t block_s = 2*max_target_length - (2*max_target_length % blockDim.x); block_s >= 0; block_s -= blockDim.x) {
    ...
    for (int64_t t=max_input_length-2; t>=0; t--) {
      ...
      if ((t < input_length - 1) && (s < 2 * target_length + 1)) {
        ...
        //概率大于log(0.1),即-2.302585时
        if(log_probs_data[lp_batch_offset + t * lp_input_stride + lp_char_stride * current_target_prime]>-2.302585)
        {
          char_frequency_data[char_data_batch_offset + fre_char_stride*current_target_prime]++;
        }
        ...
        if (current_target_prime!=BLANK && char_frequency_data[char_data_batch_offset+fre_char_stride*current_target_prime]>char_es_data[char_data_batch_offset+fre_char_stride*current_target_prime]){
          

          float po_regu = poisson_regular(char_frequency_data[char_data_batch_offset+current_target_prime*fre_char_stride],char_es_data[char_data_batch_offset+current_target_prime*fre_char_stride]);

          log_beta_data[lb_batch_offset + lb_input_stride * t + lb_target_stride * s] +=po_regu;    
