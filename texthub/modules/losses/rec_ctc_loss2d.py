...
class CTCLoss2D(nn.Module):
    ...
    def expand_with_blank(self, targets:torch.Tensor):
        N, S = targets.shape
    ...
    def forward(self, mask, classify, targets, input_lengths, target_lengths):
        ...
        probability = torch.log((torch.zeros(S, H, N).to(device) + tiny) / H).to(device)
        probability[0] = classify[0, :, :, self.blank]
        # print("first 0,2",classify.shape,torch.max(targets_indices[:, :, 1:2]).data)
        ...
        for timestep in range(1, T):
            ...
            # print("second 0,2:",timestep,torch.max(targets_indices).data)
        ...
        # lengths = (target_lengths * 2 + 1).type(torch.int64)
        lengths = (target_lengths*2 +1).unsqueeze(0).type(torch.int64)
        # print("end",probability.shape,torch.max(lengths-1).data,torch.max(lengths-2).data)

        ##TODO：此处会触发数组越界length 越界问题
